...
def TransformToProb(meanv, sigma2, mu):
	...
	for t in range(T):
		...
		ss        = np.random.normal(meanv[t], sigma[t], NUM_SAMPS)
		ps        = np.exp(ss + mu)/(1.0+np.exp(ss + mu))
...
def BackwardFilter(x_post,x_prior,sigma2_post, sigma2_prior):
	...
	for t in range(T-1, 0, -1):  ##ACS changed here
